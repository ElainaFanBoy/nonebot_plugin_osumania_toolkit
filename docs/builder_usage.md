...
该 runner 直接调用 Etterna MinaCalc，能提供与官方更接近的 ETT 结果。当 Etterna MinaCalc 更新后，建议重新编译 runner，避免插件结果与新版本 Etterna 偏离。如果Etterna官方修改了MinaCalc的输入输出格式，可能还需要对`official_minacalc_runner.cpp`进行相应调整。
...
如果目录不是 `etterna-master`，请改名，否则脚本会报找不到 `MinaCalc.cpp`，或手动修改脚本内容。
...
- `src/nonebot_plugin_osumania_toolkit/algorithm/ett/official_minacalc_runner.exe`
- 或 `src/nonebot_plugin_osumania_toolkit/algorithm/ett/official_minacalc_runner`

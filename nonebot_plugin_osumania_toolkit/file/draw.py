...
def _has_scorev2_mod(osr_obj: Optional[osr_file]) -> bool:
    if osr_obj is None:
        return False
...
def _build_default_rulesets(osu_obj: Optional[osu_file], osr_obj: Optional[osr_file]) -> list[ruleset_file]:
    if osu_obj is None:
        return []
...
def _match_for_visualization(osu_obj: osu_file, osr_obj: osr_file) -> tuple[list[tuple[int, float]], list[tuple[int, float, float]]]:
    fallback_delta: list[tuple[int, float]] = []
    fallback_pairs: list[tuple[int, float, float]] = []

    for ruleset_obj in _build_default_rulesets(osu_obj, osr_obj):
...
def plot_delta(osr_obj: osr_file, osu_obj: osu_file, output_dir: str):
    ...
    output_dir: 输出目录
    """
    delta_list, _matched_pairs = _match_for_visualization(osu_obj, osr_obj)
...
def plot_scatter(osr_obj: osr_file, osu_obj: osu_file, output_dir: str) -> str:
    ...
    _delta_list, matched_pairs = _match_for_visualization(osu_obj, osr_obj)
...
def plot_comprehensive(output_dir: str, osr_obj: osr_file, osu_obj: osu_file = None) -> str:
    ...
    if osu_obj is not None:
        delta_list, matched_pairs = _match_for_visualization(osu_obj, osr_obj)
    else:

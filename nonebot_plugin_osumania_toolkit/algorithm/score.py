...
from ..file.osu_file_parser import NOTE_HOLD_HEAD, NOTE_HOLD_TAIL, NOTE_NORMAL
...
def _evaluate_lamps(
    ...
    for lamp in lamps_config:
        ...
        if "ComboBreaksAtMost" in req:
            ...
            ok = isinstance(limit, int) and combo_breaks <= limit
            lamps[name] = ok
            if ok and isinstance(limit, int):
        ...
        if "JudgementAtMost" in req:
            ...
            if isinstance(entry, list) and len(entry) == 2 and isinstance(entry[0], int) and isinstance(entry[1], int):
                ...
                if 0 <= j_idx < len(judgement_counts):
                    ok = judgement_counts[j_idx] <= limit
                    if ok:
...
def _pick_grade(grades_config: list[dict[str, Any]], accuracy: float) -> tuple[str, float]:
    grades: list[tuple[float, str]] = []
    for item in grades_config:
        if not isinstance(item, dict):
            continue
        threshold = item.get("Accuracy")
        name = str(item.get("Name", "")).strip()
        if _is_number(threshold) and name:
            grades.append((float(threshold), name))

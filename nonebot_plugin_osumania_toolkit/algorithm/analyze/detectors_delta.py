...
from .detectors_delta_memory import detect_ar1_memory_pattern, detect_fatigue_trend, detect_nonlinear_memory
...
def analyze_delta_t(osr_obj: osr_file, osu_obj: osu_file) -> dict:
    ...
    # 暂时不启用长空段上下文检测，误判较为严重，后续根据需要调整参数和启用条件
    # replay_events = getattr(osr_obj, "press_events_float", None) or osr_obj.press_events
    # gap_sig = detect_gap_ghost_context_v2(replay_events, osu_obj.note_times, matched_pairs)
    # if gap_sig is not None:
    #     sus = True
    #     risk_score += gap_sig.risk
    #     signals.append(gap_sig)
    #     reasons.append(gap_sig.reason)
